...
void Node::printNode() {
	...
	for (size_t i = 0; i < size; i++) {
		...
		cout << this->package_node[i].getSize() << " " << this->package_node[i].getSender() << " " << this->package_node[i].getData() << " "
		     ...
		     << this->package_node[i].getFormat() << std::endl;
	}
}

class Package& Node::addPackage(size_t pos, Package *element) {
...
if (pos > this->size) {
	...
	memcpy(new_pack + pos + 1, this->packages_node + pos, (this->size - pos) * sizeof(Packages));
	violin_node = new_pack;
	this->size++;
	std::cout << "Yes, you add package" << pos << std::endl;
	return *package_node;
}
...
class Packages &Node::findElementPackages(size_t pos) {
	if (pos >= size || pos < 0) {
		...
		std::cout << "Yrs, you find package" << pos << std::endl;
	return package_node[pos];
}

void Node::delElementPackages(const size_t pos) {
	Violins *new_pack = new Packages[size - 1];
...
class Packages &Node::findPackagesold() {
	for (unsigned long int i = 0; i < size; i++) {
		for (unsigned long int j = 0; j < size - i - 1; j++) {
			...
			if (package_node[i].creator == "Nova Poshta") {
				std::cout << "The oldest package is" << std::endl;
				cout << this->package_node[i].getSize() << " " << this->package_node[i].getSender() << " "
				     << this->package_node[i].getData() << " " << this->package_node[i].bow.getBow_weight() << " "
				     << this->package_node[i].bow.getBow_cost() << " " << this->package_node[i].getFormat() << std::endl;
			}
		}
	}
	return *package_node;
...
void Node::writeFile() {
	...
	for (unsigned int i = 0; i < size; i++) {
		file << this->package_node[i].getSize() << " " << this->package_node[i].getSender() << " " << this->package_node[i].getData() << " "
		     << this->package_node[i].bow.getBow_weight() << " " << this->package_node[i].bow.getBow_cost() << " "
		     << this->package_node[i].getFormat() << std::endl;
	}
	file.close();
